...
## 14. First live graded outcomes, 2026-09-21 — the forecast measured against reality

Everything before this section is backtest, walk-forward replay, or a sample
of two. This is the first substantial set of predictions that were **logged
before the outcome existed** and then graded against what actually happened.
It is the number the whole system has been built to produce, so it is reported
here without softening.

### 14.1 What was graded

`evening-verify-predictions` ran on schedule at 18:30 IST (49.2s, status
`success`) — its first ever completed run under durable logging. It graded the
1-day horizon batch predicted from the 2026-09-18 close against the 2026-09-19
outcome.

- **154 graded 1-day predictions** (plus 2 pre-existing, 156 total in the ledger)
- 73 correct, 81 wrong
- **Direction hit rate: 47.4%**
- Mean absolute error: 1.34 percentage points

The remaining 614 predictions (3d/7d/15d/30d) are correctly still pending —
their horizons have not elapsed.

### 14.2 The headline: no demonstrated edge on direction

**47.4% on n=154 is below a coin flip.** With that sample the 95% interval
comfortably spans 50%, so this is not evidence of *negative* skill — it is
evidence of **no skill**. The honest reading is that one-day direction
remains unpredictable by this model, exactly as §11 and the public track
record have always claimed.

Both sides of the call underperformed, so this is not a one-directional bias:
...
### 14.3 The probabilities are inverted where it matters most

This is the more serious finding, and it is not visible from the hit rate
alone. Bucketing by the predicted probability of an up-move:

| Predicted P(up) | n | Mean predicted | **Observed up-frequency** |
|---|---|---|---|
| ≥ 0.525 (confident UP) | 22 | 0.550 | **0.273** |
| 0.475–0.525 (neutral) | 65 | 0.496 | 0.538 |
| < 0.475 (confident DOWN) | 67 | 0.443 | 0.522 |

Where the model was most confident of an up-move it was right **27%** of the
time; where it leaned down, the market went up 52% of the time. The ordering
is backwards: confidence is anti-correlated with being right. On this sample
the confident-UP bucket is only n=22, so this is a signal to investigate, not
an established fact — but it is precisely the pattern that makes a
probability actively misleading rather than merely uninformative, and it must
not be published to a user until it is understood.

This is what the calibration layer exists to catch, and it is why the
promotion gate refuses calibrators that do not improve held-out Brier.

### 14.4 The model systematically under-forecasts magnitude

| Statistic | Predicted | Actual |
|---|---|---|
| Median absolute move | **0.139%** | **1.140%** |
| 90th percentile absolute move | — | 2.653% |
| Largest absolute move | — | 8.28% |

The model predicts moves roughly **8× smaller** than the market delivers. The
worst single miss: OFSS called UP at 54.5% confidence with +0.23% expected;
it closed **−8.28%**, a 8.51pp error.

Under-forecasting volatility this severely has a direct safety consequence:
position sizing and stop placement derived from these expected moves would be
far too tight for the moves that actually occur. Any sizing logic consuming
`expected_return` or the 80% band must be treated as unvalidated until this
is corrected.

### 14.5 What this changes

Nothing is promoted, and nothing about the system's authority ceilings
changes. What changes is that the claims in §11 are no longer only
backtest-supported — they now have prospective, out-of-sample confirmation:

- Direction ≈ coin flip: **confirmed live** (47.4%, n=154).
- 0/12 setups with a positive edge (§13.1): **consistent with this result.**
- The ranges, not the direction, are the product: **now in doubt too**, because
  §14.4 shows the magnitude model is badly miscalibrated on live data.

The three concrete follow-ups, in priority order:
1. Re-fit calibrators against this live batch and check whether the inversion
   in §14.3 survives a larger sample. Do not ship a calibrator on n=22.
2. Investigate the magnitude under-forecast (§14.4) before any sizing logic
   consumes these expected moves.
3. Let the 3d/7d/15d/30d horizons mature and re-read. One day is one day;
   614 predictions are still outstanding.
